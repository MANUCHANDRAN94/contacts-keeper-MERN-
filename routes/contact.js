...
router.put('/:id', auth, async (req, res) => {
    ...
    try {
        ...
        if (!contact) return res.status(404).json({ msg: 'Comntact not found' });
        ...
        contact = await Contact.findByIdAndUpdate(req.params.id,
            { $set: contactFields },
            { new: true }); //! if this contact doesnt exists it will create a new one
...
router.delete('/:id', auth, async (req, res) => {
    try {
        ...
        await Contact.findByIdAndRemove(req.params.id);  //todo findByIdAndDelete is deprecated
